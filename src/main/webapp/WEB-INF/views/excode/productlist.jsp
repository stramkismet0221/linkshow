...
<div id="product_content">
    ...
    <div class="modal-body" style="width: 900px;overflow:auto;padding-bottom: 0;margin-bottom: -15px;" >
        <div class="row">
            ...
            <div class="col-md-3">
                <input type="text" class="form-control" name="keyword1" maxlength="50" placeholder="请输入查询条件" value="${keyword}"/>
            </div>
...
<script type="text/javascript">
    ...
    jQuery(document).ready(function() {
        var checked = $("input[name='cc']:checkbox:checked").not("input[id='checkbox4']").length;
    ...
    function skipPage1(page) {
        ...
        $("input[name='cc']:checkbox:checked").each(function(){
        ...
        $.ajax({
            url:'${pageContext.request.contextPath}/excode/getproductlist',
            type:'post',
            dataType:'html',
            data:{"totalPage":totalPage, "page":page,"barCodes":barCodes,"keyword":keyword},
    ...
    function saveProduct() {
        ...
        var tableDate = "";
        var tbody = $("#product_content").parents(".col-md-8").find("tbody[id='product_tbody']").html();
        $("input[name='cc']:checkbox:checked").each(function(){
            var data = $(this).parents("tr").children("td").children("input[id='checkbox4']").context.id;
            if (data === "checkbox4"){
                return true;
            ...
            tableDate += "<tr>" +
                ...
                "<input type=\"number\"  min='1' max='999999' oninput=\"if(value.length>=6)value=value.slice(0,6)\"   style='padding-left: 10px;' name=\"amount\" class=\"form-control\" placeholder=\"请输入数量\">" +
                "</div>" +
                "</td>" +
        ...
        $("#product_content").parents().find("input[name='barCodes']").val(productids);
        $("#product_content").parents(".col-md-8").find("tbody[id='product_tbody']").append(tableDate);
    ...
    function search() {
        ...
        var barCodes = $("input[name='productIds']").val();
        $("input[name='cc']input[name='cc']:checkbox:checked").each(function(){
            var data = $(this).parents("tr").children("td").children("input[id='checkbox4']").context.id;
            if (data === "checkbox4"){
                return true;
            }
        ...
        $.ajax({
            ...
            type:'post',
            dataType:'html',
            data:{"keyword":keyword,"barCodes":barCodes},
            success:function (data) {
                $("#product_content").html(data);
            }
        })
    ...
    function chooseOne() {
        var checked = $("input[name='cc']input[name='cc']:checkbox:checked").not("input[id='checkbox4']").length;
        var count = $("#product tbody tr").length;
        if (count === checked){
            $("input[id='checkbox4']").prop("checked","true");
            index = 1;

        ...
        <div class="panel">
            <div class="table-responsive">
                <table class="table table-hover manage-u-table" style="white-space: nowrap">
                    ...
                    <tbody>
                    <c:forEach items="${unCheckedUser}" var="user">
                        <tr>
                            <td>${user.id}</td>
                            ...
                            <td>
                                <div>
                                    <div style="display: inline-block; width:50%">
                                        <input type="button" class="btn btn-block btn-info btn-xs" value="加入" onclick="adduserpower(${user.id});"/>
                                    </div>
                                    &nbsp;
                                </div>
                            </td>
...
<script type="text/javascript">
    ...
    function skipPage(page) {
        ...
        var url = "${pageContext.request.contextPath}/power/unselectuser";
    ...
    //权限加入用户
    function adduserpower(userId) {
        var systemId = $("input[name='systemId']").val();
        var powerId = $("input[name='powerId']").val();
        $.ajax({
            url : "${pageContext.request.contextPath}/power/adduserpower",
            type : "POST",
            dataType : "json",
            data : {"userId":userId,"powerId":powerId,"systemId":systemId},
            success : function(result) {
                if (result.success) {
